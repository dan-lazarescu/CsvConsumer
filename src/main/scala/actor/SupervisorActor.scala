...
class SupervisorActor(implicit val cassandraSession: CassandraSession,
  ...
  val logger = LoggerFactory.getLogger(classOf[SupervisorActor])
  val actorDb = context.actorOf(model.ItemsRepo.props(cassandraSession), "DbActor")
  val actorKafka = context.actorOf(model.KafkaConsumerActor.props(cassandraSession), "KafkaActor")
